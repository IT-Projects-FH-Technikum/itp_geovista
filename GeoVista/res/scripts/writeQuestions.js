...
function isQuizFinished(questions) {
...
function createQuestion(questions) {
    ...
    const quizID = ((_a = document.getElementById('quizId')) === null || _a === void 0 ? void 0 : _a.getAttribute('data-quizId')) || '';
...
function evaluateQuestion(checkQuestionButton, answerSection, questions) {
    const mainContent = document.getElementById('mainContent');
    const radios = answerSection.querySelectorAll('input[type="radio"]'); //Select all radio buttons in the form
...
function createQuizEvaluation(questions) {
    const mainContent = document.getElementById('mainContent');
    mainContent.innerHTML = ""; //Clear the main content
    const percentScore = (countCorrectAnswers / questions.length) * 100;
    const passingText = percentScore >= 50 ? "Gut gemacht, du hast bestanden!" : "Leider hast du nicht bestanden! Nächstes Mal wird es besser!";
    const passingIcon = percentScore >= 50 ? "✔" : "✘";
    const passingClass = percentScore >= 50 ? "text-success" : "text-danger";
    const evaluationText = document.createElement("div");
